import { safeSpot, startingPoint, turningPoint, victoryStart } from "@/Utils/PlotData"
import { selectCurrentPostions, selectDiceNo } from "./gameSelection"
import { announceWinner, disableTouch, unfreezDice, updatePlayerChance, updatePlayerPieceValue } from "./gameSlice"
const delay = (ms: any) => new Promise(resolve => setTimeout(resolve, ms));

export const handleForwardThunk = (playerNo: any, id: any, pos: any) => async (dispatch: any, getState: any) => {
    ...
    const piecesAtPostion = plotedPieces?.filter((item: any) => item.pos === pos)

    const piece = piecesAtPostion[
    ...
    const beforePlayerpiece = state.game[`player${playerNo}`].find((item: any) => item.id == id)
    ...
    // Check collision
    const finalPlot = updatedPlottedPieces?.filter((item: any) => item.pos === finalPath)
